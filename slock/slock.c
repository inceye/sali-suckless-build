...
dontkillme(void)
{
	...
	static void
writemessage(Display *dpy, Window win, int screen, int pwlen)
{
	int len, line_len, width, height, s_width, s_height, i, j, k, fink, tab_replace, tab_size;
	XGCValues gr_values;
...
#ifdef PWDIND
	char pwspholder [pwlen];
	for (i = 0; i < pwlen; i++) {
		pwspholder[i] = '*';
	}
	XDrawString(dpy, win, gc, width, height + 20 * (fink + 1), pwspholder, pwlen);
#endif
